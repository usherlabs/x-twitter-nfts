...
use indexer::helper::proof::get_verity_client;
...
pub async fn verify_near_proof_v2() {
    ...
    let tweet_id = "1894760981462479090";
    ...
    let transaction = Transaction {
        ...
        actions: vec![Action::FunctionCall(Box::new(FunctionCallAction {
            ...
            args: json!({
                ...
                "image_url": "",
                "owner_address": "local-verifier.testnet"
            })
            .to_string()
            .into_bytes(),
    ...
    let response = client
        .call(methods::tx::RpcTransactionStatusRequest {
            transaction_info: TransactionInfo::TransactionId {
                tx_hash: tx_hash.expect("tx_hash"),
    ...
    let merkle_root = response
        ...
        .expect("final_execution_outcome result")
        .into_outcome()
        .status;

    println!(
        "\nverified_by_remote: {:#?}\n {:#?}",
        leaves, verified_by_remote
    );
    println!("\nresponse: {:#?}\n", merkle_root);
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_verify_near_proof_v2() {
        dotenv().expect("Error occurred when loading .env");

        let _x = verify_near_proof_v2().await;
    }
}
