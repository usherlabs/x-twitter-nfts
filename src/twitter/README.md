# TLS Proof of X (Twitter)

The sets up a TLS connection with Twitter and notarizes API requests. The full received transcript is notarized in one commitment, so nothing is redacted. The resulting proof is written to a local JSON file (`twitter_dm_proof.json`) for easier inspection.
